# Phase 4: Data Graph — Implementation Plan (v2)
...
>
> 1. **Root object changed:** `ssot__Individual__dlm` → `UnifiedssotIndividualInd1__dlm` — REQUIRED for MC Advanced (MC on Core) personalization tasks including segment-triggered Flow decisions and Data Graph-powered email merge fields.
> 2. **All `relationshipName` values updated:** Now reference `UnifiedssotIndividualInd1__dlm` instead of `ssot__Individual__dlm` (e.g., `CarMax_Vehicle__dlm_UnifiedssotIndividualInd1__dlm`).
> 3. **WebsiteEngagement field name corrected:** `ssot__EngagementChannelActionId__c` → `ssot__EngagementTypeId__c` (matches Phase 3 v2 correction).
> 4. **Org alias corrected:** `carmax-sdo-mm-app-wz95pw` → `carmax-sdo-mm-app-2fs4hp`.
> 5. **API version updated:** 65.0 → 66.0.
...
```
Unified Individual (UnifiedssotIndividualInd1__dlm)
├── CarMax Vehicle (CarMax_Vehicle__dlm)
│     └── linked via IndividualId__c → Individual.ssot__Id__c (FK chain through Identity Resolution)
├── CarMax TestDrive (CarMax_TestDrive__dlm)
│     └── linked via IndividualId__c → Individual.ssot__Id__c (FK chain through Identity Resolution)
└── Website Engagement (ssot__WebsiteEngagement__dlm)
      └── linked via ssot__IndividualId__c → Individual.ssot__Id__c (FK chain through Identity Resolution)
```
...
> - **ssot__WebsiteEngagement__dlm** — Standard ssot__ fields: `ssot__Id__c`, `ssot__EngagementTypeId__c`, `ssot__EngagementVehicleId__c`, `ssot__EngagementDateTm__c`, `ssot__PageURL__c`, `ssot__DeviceTypeTxt__c`, `ssot__UtmSourceName__c`, `ssot__UtmCampaignName__c`
...
```bash
sf api request rest "/services/data/v66.0/tooling/sobjects/DataGraphDefinition" \
  --method POST \
  --target-org carmax-sdo-mm-app-2fs4hp \
  --body '{
...
```json
{
  ...
  "primaryObjectName": "UnifiedssotIndividualInd1__dlm",
  "primaryObjectLabel": "Unified Individual",
  "label": "CarMax Customer 360",
  "name": "CarMax_Customer_360",
  "sourceObject": {
    "name": "UnifiedssotIndividualInd1__dlm",
    "type": "Standard",
    ...
    "relatedObjects": [
      {
        ...
        "relationshipName": "CarMax_Vehicle__dlm_UnifiedssotIndividualInd1__dlm",
        "fields": [
          {
      ...
      },
      {
        ...
        "relationshipName": "CarMax_TestDrive__dlm_UnifiedssotIndividualInd1__dlm",
        "fields": [
          {
      ...
      },
      {
        ...
        "relationshipName": "ssot__WebsiteEngagement__dlm_UnifiedssotIndividualInd1__dlm",
        "fields": [
          {
          ...
          },
          {
            ...
            "sourceFieldName": "ssot__EngagementTypeId__c",
            "isKeyColumn": false,
            "usageTag": "NONE",
            "dataType": "Text"
          },
...
> - The `relationshipName` for WebEng is `ssot__WebsiteEngagement__dlm_UnifiedssotIndividualInd1__dlm` (matching the actual FK relationship through to the Unified Individual root)
...
sf project deploy start --source-dir force-app --target-org carmax-sdo-mm-app-2fs4hp --wait 10
```
...
```bash
sf api request rest "/services/data/v66.0/tooling/query?q=SELECT+Id,DeveloperName,MasterLabel,SourceObjectType+FROM+DataKitObjectTemplate+WHERE+MasterLabel='CarMax_Customer_360'" \
  --target-org carmax-sdo-mm-app-2fs4hp
```
...
```bash
sf api request rest "/services/data/v66.0/tooling/query?q=SELECT+Id,EntityPayload+FROM+DataKitObjectTemplate+WHERE+MasterLabel='CarMax_Customer_360'" \
  --target-org carmax-sdo-mm-app-2fs4hp 2>&1 | grep -v "^Warning:" | python3 -c "
import json, sys
...
Root: UnifiedssotIndividualInd1__dlm
Type: NONE
Root fields: 4
...
sf api request rest "/services/data/v66.0/ssot/data-graphs/CarMax Customer 360/actions/refresh" \
  --method POST \
  --target-org carmax-sdo-mm-app-2fs4hp \
  --body '{}'
```
...
|----------------|-----------|---------|
| Unified Individual (root) | Stop 3 | Customer name/greeting in emails |
...
1. Queries the CarMax Customer 360 graph for Jane Dawson's record via the Unified Individual root
...
4. **`sourceObject.type` values** — `"Standard"` for ssot__ DMOs (Individual, WebsiteEngagement) and for `UnifiedssotIndividualInd1__dlm` (the Unified Individual from Identity Resolution). `"Custom"` for custom DMOs (Vehicle, TestDrive). Capitalize the first letter.
...
7. **`relationshipName` convention** — `{ChildDMO}_{RootDMO}` (e.g., `CarMax_Vehicle__dlm_UnifiedssotIndividualInd1__dlm`). This must match an existing FK relationship between the DMOs and the Unified Individual root.
...
14. **Unified Individual MUST be the root for MCA personalization** — For the Data Graph to be usable in MC Advanced (MC on Core) personalization — including segment-triggered Flows and Data Graph-powered email merge fields — the root object MUST be the Unified Individual DMO (`UnifiedssotIndividualInd1__dlm`) from Identity Resolution. Using `ssot__Individual__dlm` (raw Individual) as the root will make the graph unusable for MCA personalization tasks.
