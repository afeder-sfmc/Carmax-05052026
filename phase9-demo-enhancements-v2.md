...
1. **Org alias corrected** to `carmax-sdo-mm-app-2fs4hp` throughout all queries and scripts
2. **WebEng field name corrected:** `ssot__EngagementChannelActionId__c` → `ssot__EngagementTypeId__c` in Enhancement #5 SSOT query (cascading from Phase 3 v2)
3. **Demo Stop 3 narrative updated** for three repeater types: hearted vehicles, purchased vehicles, and test drives (cascading from Phase 7 v2)
4. **Data Graph root object** references updated to `UnifiedssotIndividualInd1__dlm` where applicable in demo narratives
...
- Purchased vehicle cards rendered from Data Graph repeater (green header, 'Schedule Service' CTA)
- Test drive history rendered from Data Graph repeater (store, date, outcome)
...
The Data Graph (rooted on Unified Individual -- `UnifiedssotIndividualInd1__dlm`) must be built and operational for the dynamic loop to render vehicle cards:
...
2. Explain the graph structure: Unified Individual > Vehicle, Test Drive, Web Engagement
...
1. **Preview as Jane Dawson:**
   ...
   - Purchased vehicle section shows her CarMax vehicle history with green header and 'Schedule Service' CTA
   - Test drive section shows her Ford Expedition test drive at CarMax Richmond
   - Pre-qualification block is HIDDEN (she already pre-qualified)
   - CTA: "Schedule a Test Drive"
...
- "The email has THREE dynamic sections -- hearted vehicles, purchased vehicles, and test drives -- each pulling from different related objects on the graph"
...
sf api request rest '/services/data/v58.0/ssot/query' \
  --method POST \
  --target-org carmax-sdo-mm-app-2fs4hp \
  --body '{"sql":"SELECT ssot__EngagementTypeId__c, ssot__EngagementVehicleId__c, ssot__EngagementDateTm__c, ssot__PageURL__c FROM ssot__WebsiteEngagement__dlm WHERE ssot__IndividualId__c = '"'"'JANE_INDIVIDUAL_ID'"'"' ORDER BY ssot__EngagementDateTm__c DESC LIMIT 10"}'
```

**Expected:** Multiple rows showing diverse event types via `ssot__EngagementTypeId__c` (Vehicle Detail View, Heart Vehicle, Schedule Test Drive, Pre-Qual Complete, etc.).
...
for CI_NAME in Customer_Lifetime_Value Propensity_to_Buy Vehicle_Preference_Affinity Engagement_Velocity; do
  ...
  sf api request rest \
    ...
    --target-org carmax-sdo-mm-app-2fs4hp 2>/dev/null | python3 -c "
import json, sys
data = json.load(sys.stdin)
for row in data.get('data', []):
...
The Data Graph (rooted on Unified Individual -- `UnifiedssotIndividualInd1__dlm`) provides the visual relationship view on the 360 profile:
...
3. **Show the Data Graph:** Unified Individual > Vehicle, Test Drive, Web Engagement relationships
...
| Data Graph (root: `UnifiedssotIndividualInd1__dlm`) | Phase 4 | Built | Reflects updated vehicle data |
