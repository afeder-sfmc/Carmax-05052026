...
1. **Field name correction:** All segment SQL references to `ssot__EngagementChannelActionId__c` changed to `ssot__EngagementTypeId__c` (cascading from Phase 3 v2 correction). Affects 3 Dbt segments: Pre-Qualification Shoppers, Instant Offer Abandonment, Saved Search Match.
2. **Org alias corrected** to `carmax-sdo-mm-app-2fs4hp`
3. **API version updated** from 65.0 to 66.0
...
```bash
sf apex run --target-org carmax-sdo-mm-app-2fs4hp --file segment_create.apex
```
...
node.sql = ''
    ...
    + 'INNER JOIN ssot__WebsiteEngagement__dlm we '
    + '  ON we.ssot__IndividualId__c = raw.ssot__Id__c '
    + 'WHERE we.ssot__EngagementTypeId__c = \'Pre-Qualification Start\' '
    ...
    + '    WHERE we2.ssot__EngagementTypeId__c = \'Pre-Qualification Complete\' '
    + '  )';
...
node.sql = ''
    ...
    + 'INNER JOIN ssot__WebsiteEngagement__dlm we '
    + '  ON we.ssot__IndividualId__c = raw.ssot__Id__c '
    + 'WHERE we.ssot__EngagementTypeId__c = \'Instant Offer Start\' '
    ...
    + '    SELECT we2.ssot__IndividualId__c '
    + '    FROM ssot__WebsiteEngagement__dlm we2 '
    + '    WHERE we2.ssot__EngagementTypeId__c = \'Instant Offer Complete\' '
...
node.sql = ''
    ...
    + 'WHERE we.ssot__EngagementTypeId__c = \'Save Search\' '
    + '  AND v.Status__c = \'Active\' '
    + '  AND YEAR(we.ssot__EngagementDateTm__c) * 10000 + MONTH(we.ssot__EngagementDateTm__c) * 100 + DAY(we.ssot__EngagementDateTm__c) >= 20260411 ';
...
sf api request rest \
  --method POST \
  --target-org carmax-sdo-mm-app-2fs4hp \
  "/services/data/v66.0/ssot/segments/<segment-id>/publish"
...
ORG="carmax-sdo-mm-app-2fs4hp"
API="/services/data/v66.0/ssot/segments"
    ...
    if dev_name.startswith(('Vehicle_Interest', 'New_Email', 'Pre_Qualification', 'Price_Drop', 'Instant_Offer', 'Saved_Search', 'EV_Consideration', 'Trade_In')):
        ...
        result = subprocess.run(
            ['sf', 'api', 'request', 'rest', '--method', 'POST',
             '--target-org', '$ORG',
             f'/services/data/v66.0/ssot/segments/{seg_id}/publish'],
            capture_output=True, text=True
        )
...
sf api request rest \
  --method GET \
  --target-org carmax-sdo-mm-app-2fs4hp \
  "/services/data/v66.0/ssot/segments/<waterfall-segment-id>/members?limit=50"
```
...
```
/services/data/v66.0/ssot/segments/{segmentId}/publish
```
...
sf api request rest \
  --method DELETE \
  --target-org carmax-sdo-mm-app-2fs4hp \
  "/services/data/v66.0/ssot/segments/<segment-id>"
```
