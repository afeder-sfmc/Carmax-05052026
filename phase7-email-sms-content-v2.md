...
1. **Root object corrected:** All references to `ssot__Individual__dlm` as the Data Graph root changed to `UnifiedssotIndividualInd1__dlm` — required for MC Advanced personalization
2. **Purchased vehicle repeater added:** New `{{#each CX360_Vehicle}}{{#if CX360_Vehicle_IsPurchased}}` section in Email #1 showing purchased vehicles with green header and "Schedule Service" CTA
3. **Test drive repeater added:** New `{{#each CX360_TestDrive}}` section in Email #1 showing test drive history with store, date, VIN, and "Book Another Test Drive" CTA
4. **Field name correction:** `ssot__EngagementChannelActionId__c` → `ssot__EngagementTypeId__c` in the Web Engagement field mapping table (cascading from Phase 3 v2)
5. **IsHearted__c field name corrected** in mapping table (was `Is_Hearted__c`, should be CamelCase `IsHearted__c`)
6. **Org alias corrected** to `carmax-sdo-mm-app-2fs4hp`
...
Root Object:  UnifiedssotIndividualInd1__dlm
Related Object: CarMax_Vehicle__dlm
Relationship: CarMax_Vehicle__dlm_UnifiedssotIndividualInd1__dlm
...
**Purchased Vehicle Repeater:**
```
Data Source:  CarMax_Customer_360 graph (Phase 4)
Root Object:  UnifiedssotIndividualInd1__dlm
Related Object: CarMax_Vehicle__dlm
Relationship: CarMax_Vehicle__dlm_UnifiedssotIndividualInd1__dlm
Filter:       CX360_Vehicle_IsPurchased = true
```
...
Data Source:  CarMax_Customer_360 graph (Phase 4)
Root Object:  UnifiedssotIndividualInd1__dlm
Related Object: CarMax_TestDrive__dlm
Relationship: CarMax_TestDrive__dlm_UnifiedssotIndividualInd1__dlm
```
...
<div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;">
  ...
  <!-- Purchased Vehicle History Header -->
  <div style="padding: 24px 32px 8px 32px;">
    <h2 style="color: #003468; font-size: 20px; font-weight: 700; margin: 0;">
      Your CarMax Vehicles
    </h2>
    <p style="color: #666666; font-size: 14px; margin: 4px 0 0 0;">
      Vehicles you've purchased from CarMax
    </p>
  </div>

  <!-- Purchased Vehicle History Section -->
  {{#each CX360_Vehicle}}
    {{#if CX360_Vehicle_IsPurchased}}
    <div style="margin: 0 32px 16px 32px; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">

      <!-- Vehicle Header — Purchased -->
      <div style="background-color: #1a6b3c; padding: 12px 16px; display: flex; justify-content: space-between; align-items: center;">
        <span style="color: #ffffff; font-size: 18px; font-weight: 600;">
          {{CX360_Vehicle_Year}} {{CX360_Vehicle_Make}} {{CX360_Vehicle_Model}}
        </span>
        <span style="color: #ffffff; font-size: 12px; background-color: rgba(255,255,255,0.2); padding: 4px 8px; border-radius: 4px;">
          PURCHASED
        </span>
      </div>
      ...
      <div style="padding: 16px;">
        ...
        <!-- Service CTA -->
        <div style="margin-top: 12px; text-align: center;">
          <a href="https://www.carmax.com/car-care" style="display: inline-block; background-color: #1a6b3c; color: #ffffff; padding: 10px 28px; border-radius: 4px; text-decoration: none; font-size: 14px; font-weight: 600;">
            Schedule Service
          </a>
        </div>
      </div>
  ...
  <!-- Test Drive History Header -->
  <div style="padding: 24px 32px 8px 32px;">
    <h2 style="color: #003468; font-size: 20px; font-weight: 700; margin: 0;">
      Your Recent Test Drives
    </h2>
    <p style="color: #666666; font-size: 14px; margin: 4px 0 0 0;">
      Vehicles you've test driven at CarMax
    </p>
  </div>

  <!-- Test Drive Cards — Repeater Block -->
  {{#each CX360_TestDrive}}
  <div style="margin: 0 32px 16px 32px; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">

    <!-- Test Drive Header -->
    <div style="background-color: #003468; padding: 12px 16px; display: flex; justify-content: space-between; align-items: center;">
      <span style="color: #ffffff; font-size: 16px; font-weight: 600;">
        Test Drive at CarMax {{CX360_TestDrive_Store}}
      </span>
      <span style="color: #ffffff; font-size: 12px; background-color: rgba(255,255,255,0.2); padding: 4px 8px; border-radius: 4px;">
        {{CX360_TestDrive_Date}}
      </span>
    </div>
    ...
    <div style="padding: 16px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse;">
        <tr>
          <td style="padding: 4px 8px; color: #666666; font-size: 13px; width: 50%;">Vehicle VIN</td>
          <td style="padding: 4px 8px; color: #333333; font-size: 13px; font-weight: 600; width: 50%;">{{CX360_TestDrive_VIN}}</td>
        </tr>
        <tr>
          <td style="padding: 4px 8px; color: #666666; font-size: 13px;">Outcome</td>
          <td style="padding: 4px 8px; color: #333333; font-size: 13px; font-weight: 600;">{{CX360_TestDrive_Outcome}}</td>
        </tr>
      </table>

      <!-- Book Another Test Drive CTA -->
      <div style="margin-top: 12px; text-align: center;">
        <a href="https://www.carmax.com/schedule-test-drive" style="display: inline-block; background-color: #003468; color: #ffffff; padding: 10px 28px; border-radius: 4px; text-decoration: none; font-size: 14px; font-weight: 600;">
...
| Purchased Vehicle Section | Dynamic Loop | `{{#each CX360_Vehicle}}` with `{{#if CX360_Vehicle_IsPurchased}}` filter |
| Test Drive Section | Dynamic Loop | `{{#each CX360_TestDrive}}` — iterates over test drive related objects |
...
6. "Notice the email now has THREE dynamic sections: hearted vehicles, purchased vehicles, and test drives."
7. "The purchased vehicle section uses a green header to differentiate from active shopping — and offers a service CTA instead of a sales CTA."
8. "Test drives pull from a completely different related object on the graph — CarMax_TestDrive__dlm — showing the power of multi-object personalization."
...
|-------------------|---------------|-----------|-------------|---------|
| `{{CX360_First_Name}}` | CX360_First_Name | UnifiedssotIndividualInd1__dlm | ssot__FirstName__c | All 8 emails, all 4 SMS |
| `{{CX360_Last_Name}}` | CX360_Last_Name | UnifiedssotIndividualInd1__dlm | ssot__LastName__c | (Available, not used in templates) |
| `{{CX360_PersonName}}` | CX360_PersonName | UnifiedssotIndividualInd1__dlm | ssot__PersonName__c | (Available, not used in templates) |
...
| `{{CX360_Vehicle_IsHearted}}` | CX360_Vehicle_IsHearted | CarMax_Vehicle__dlm | IsHearted__c | Email #1 (loop filter) |
...
| Token in Template | Graph devName | Source DMO | Source Field | Used In |
|-------------------|---------------|-----------|-------------|---------|
| `{{CX360_WebEng_EventType}}` | CX360_WebEng_EventType | ssot__WebsiteEngagement__dlm | ssot__EngagementTypeId__c | (Available for future use) |
